/**
 * Deploy OctToken (OP-20) to OPNet testnet
 *
 * Run: npm run deploy:oct
 *
 * Prerequisites:
 *  - .env with EC_PRIVATE_KEY (64 hex) and MLDSA_PRIVATE_KEY (5120 hex) set
 *  - build/OctToken.wasm must exist (run: npm run build:oct)
...
const GAS_SAT_FEE  = 10_000n;  // increased for contract deployment gas requirements
...
console.log('=== OctToken Deployment ===');
...
const wasmPath   = join(__dirname, '..', 'build', 'OctToken.wasm');
...
console.log('Contract address:', result.contractAddress);
console.log('Contract pubkey :', result.contractPubKey);
console.log('\nSave the contract address — you will need it to interact with OctToken.');
